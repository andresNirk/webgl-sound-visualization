...
void main() {
    ...
    if (uv.y < 0.25) {
        ...
        newPosition.xyz += noise3(previousPosition.xyz*0.1)*deltaTime*60.0*(1.0+fftValue*4.0);
        ...
        fftPosition = pow(fftPosition, 3.0);
        float fftValue = abs(texture2D(fftTexture, vec2(fftPosition, 0.5) ).r);
        fftValue = pow(fftValue, 3.0-fftPosition*2.0);
        
        if (length(previousPosition.xyz) < 100.0) {
            previousSpeed.xyz = vec3(0.0, 0.0, 0.0);
        }
        if (length(previousPosition.xyz) < fftValue*200.0+100.0) {
            previousSpeed.xyz = (normalize(previousPosition.xyz)*(fftValue*200.0+100.0)-previousPosition.xyz)*0.5;
        }

        previousSpeed.xyz *= 1.0-(deltaTime*0.15);
        previousSpeed.xyz += noise3(previousPosition.xyz*0.1)*deltaTime*1.0;
        previousSpeed.xyz -= previousPosition.xyz*deltaTime*0.5*(1.0+previousPosition.a*0.1)*0.3;
        ...
        vec4 previousPosition = texture2D(texture, uv+vec2(0.0, -0.5));
        vec4 previousSpeed = texture2D(texture, uv+vec2(0.0, -0.25));
        ...
        previousValue.x = fftPosition;
        previousValue.y = fftValue;
...
void main() {
    ...
    gl_FragColor = vec4( abs(noise3(dataPosition.xyz*0.01)+dataSpeed.w*dataMisc.x*1.0*10.0)*(0.3+dataSpeed.w*2.0)*1.0, 0.0);
...
exports.NAME = 'Sphere Visualization 2';